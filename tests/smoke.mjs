...
try {
  ...
  await shareRecord;
  await page.getByRole('dialog').waitFor({ state: 'hidden' });
  const afterShare = (await workspace()).quotes.find((item) => item.id === quoteId);
  check(afterShare.status === 'sent', 'Opening WhatsApp marks the quotation as sent');
  check(afterShare.sharedVia.includes('whatsapp'), 'WhatsApp sharing channel is recorded');
  await page.getByRole('button', { name: `Compartir ${quote.number} por correo` }).click();
  await page.getByRole('dialog', { name: 'Tu propuesta, un paso más cerca' }).waitFor();
