...
import { downloadChangesExcel, downloadChangesPdf, downloadCsv, downloadQuotePdf, downloadQuotesPdfReport, downloadSalesPdfReport } from "@/lib/export";
...
type ModalState = { type: "quote"; id?: number; customerId?: number } | { type: "detail"; id: number } | { type: "share"; id: number; channel?: ShareChannel } | { type: "thanks"; id: number } | { type: "customer"; id?: number } | { type: "delete"; entity: "quote" | "customer" | "sale"; id: number } | { type: "help" } | null;
type Toast = { message: string; type: "success" | "error"; id: number } | null;
const viewLabels: Record<View, string> = { dashboard: "Panel general", quotes: "Cotizaciones", customers: "Clientes", sales: "Ventas", reports: "Reportes", settings: "Configuración" };
const navigation = [{ view: "dashboard" as View, label: "Panel general", icon: LayoutDashboard }, { view: "quotes" as View, label: "Cotizaciones", icon: FileText }, { view: "customers" as View, label: "Clientes", icon: Users }, { view: "sales" as View, label: "Ventas", icon: ShoppingBag }, { view: "reports" as View, label: "Reportes", icon: ChartNoAxesCombined }];
const isView = (value: string | null): value is View => value !== null && ["dashboard", "quotes", "customers", "sales", "reports", "settings"].includes(value);
...
export function Workspace({ initialData, initialView = "dashboard", user }: { initialData: AppData; initialView?: View; user: { email: string; name: string } }) {
  ...
  async function remove() {
    ...
    try { await mutate(modal.entity === "quote" ? "deleteQuote" : modal.entity === "sale" ? "deleteSale" : "deleteCustomer", { id: modal.id }); notify(modal.entity === "quote" ? "Cotización eliminada." : modal.entity === "sale" ? "Venta eliminada." : "Cliente eliminado."); setModal(null); }
